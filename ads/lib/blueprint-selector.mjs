...
/**
 * Alcuni blueprint sono VARIANTI di un altro (stesso pubblico/obiettivo, ma
 * destinazione diversa — es. "A-estero-spagna-web" usa destination_type
 * WEBSITE invece del modulo nativo ON_AD, per non dipendere da
 * leads_retrieval). Dichiarano `variante_di: "<id-del-blueprint-primario>"`.
 *
 * Vanno tenute FUORI dalla selezione automatica per default: un run senza
 * `--only` (launch.mjs), il calcolo del tetto aggregato (che altrimenti
 * conterebbe due volte lo stesso obiettivo commerciale), e la scelta
 * automatica di campagna_da_brief.mjs. Restano perfettamente lanciabili con
 * `--only <id-variante>`, esplicitamente.
 */
export function soloPrimari(blueprints) {
  return (blueprints || []).filter((bp) => !bp.variante_di);
}

export function soloVarianti(blueprints) {
  return (blueprints || []).filter((bp) => Boolean(bp.variante_di));
}
