...
import { lintCopyDettagliato } from './guardrails.mjs';
...
export function validaBlueprint(bp, config) {
  ...
  const { violazioni, esenzioni } = lintCopyDettagliato(bp);
  ...
  // Le esenzioni per negazione non bloccano ("non siamo un e-commerce" è
  // esattamente il messaggio che il cliente vuole), ma non sono mai silenziose:
  // chi revisiona vede sempre perché una frase con un termine vietato è passata.
  for (const e of esenzioni) {
    avvisi.push(
      `Guardrail "${e.trovato}" (categoria ${e.categoria}) trovato in ${e.percorso} ma ESENTATO: la frase lo nega (${e.estratto}). Verifica che sia davvero una negazione voluta.`
    );
  }
