...
function estraiBrand(testo) {
  const trovati = [];
  // ⚠️ Il brand va scritto nella grafia CANONICA (minuscolo, trattino), non
  // come lo scrive una persona. Misurato dal collaudo il 3/8: qui usciva
  // «BABILON» e «SHEis Color», mentre il piano editoriale usa «babilon» e
  // «sheis-color». Nessuno dei due fallisce a scrittura — ma un collegamento
  // fra campagne e contenuti per brand non trova NIENTE, e «niente» si legge
  // come «non ci sono campagne per questo brand»: la risposta sbagliata data
  // con sicurezza. La grafica leggibile resta in `match`, per chi legge.
