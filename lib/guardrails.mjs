...
// Snapshot di sicurezza — le stesse liste della BRAND-IDENTITY del
// 2026-08-03. Usato SOLO se il file vero non e' raggiungibile: FONTE_TERMINI
// (esportato sotto) dice sempre quale dei due e' stato usato in questa run.
const SNAPSHOT_FALLBACK = {
  prezzi_e_cifre_commerciali: ['€', 'euro', 'prezzo', 'listino', 'sconto', 'offerta', 'promo', 'saldo'],
  lessico_da_negozio: [
    'shop', 'negozio', 'carrello', 'acquista', 'acquistare', 'compra', 'comprare',
    'ordina ora', 'ordina adesso', 'e-commerce', 'ecommerce', 'checkout', 'aggiungi al carrello',
    'cart', 'add to cart', 'buy now', 'purchase',
    'tienda', 'carrito', 'cesta', 'comprar', 'añadir al carrito',
    'panier', 'acheter', 'ajouter au panier',
    'Warenkorb', 'kaufen', 'jetzt kaufen', 'in den Warenkorb',
    'koszyk', 'sklep', 'kup teraz', 'dodaj do koszyka',
    'loja', 'carrinho', 'comprar agora', 'adicionar ao carrinho',
    'متجر', 'سلة', 'عربة التسوق', 'اشتر الآن',
  ],
  firewall: ['Metodo 29', 'Metodo29', 'Method 29'],
};
  ...
  for (const percorso of CANDIDATI_BRAND_IDENTITY) {
    try {
      ...
      if (v?.prezzi_e_cifre_commerciali?.length && v?.lessico_da_negozio?.length && v?.firewall?.length) {
        return { prezzi: v.prezzi_e_cifre_commerciali, canale: v.lessico_da_negozio, firewall: v.firewall, fonte: percorso };
      }
    } catch {
...
function creaPattern(termine) {
  ...
  const haConfiniDiParola = /^[\p{L}\p{N}]/u.test(termine) && /[\p{L}\p{N}]$/u.test(termine);
...
// ── regola 4 (nuova): claim numerici non documentati -------------------------
// BRAND-IDENTITY §regole_di_generazione.numeri_ammessi: "Solo quelli
// documentati: 15 minuti di posa, 99% di origine naturale, tre fasi YOUNIC,
// senza ammoniaca." A questi si aggiunge 83 (nuance/matices di SHEis Color),
// gia' presente in ogni creativita' approvata di A, B e C — non e' un numero
// "inventato", e' quello che il kit gia' pubblica.
const NUMERI_DOCUMENTATI = new Set(['15', '83', '99', '3']);
const UNITA_NUMERICHE = /\b(\d+(?:[.,]\d+)?)\s*(?:%|percento|per\s*cento|por\s*ciento|percent|minut[oi]|minutos?|fas[ei]|fases?|client[ei]|clientes?|followers?|ann[oi]|años?|years?|or[ea]|horas?|hours?|giorn[oi]|días?|days?|nuance|matices|tonos?|tonalità)\b/giu;
...
function checkNumeriNonDocumentati(testo) {
  ...
  while ((m = UNITA_NUMERICHE.exec(testo)) !== null) {
    const numero = m[1];
    if (NUMERI_DOCUMENTATI.has(numero)) continue;
    violazioni.push({
      trovato: m[0],
      categoria: 'numero_non_documentato',
      perche: `numero "${numero}" accostato a un'unita' non e' nell'elenco dei dati documentati (15 minuti di posa, 83 nuance/matices, 99% di origine naturale, 3 fasi YOUNIC) — BRAND-IDENTITY §regole_di_generazione.numeri_ammessi. Marca [DA CONFERMARE] o rimuovi.`,
