...
/**
 * Valuta e fuso si scelgono UNA VOLTA alla creazione dell'account e non si
 * possono piu' cambiare: se sono sbagliati, l'account va buttato e rifatto.
 * Condivisa da lib/preflight.mjs (blocca il lancio) e attiva.mjs (si ferma
 * nel wizard prima di fare altre 20 domande su un account da rifare) — un
 * solo posto, cosi' i due punti di controllo non possono disallinearsi.
 */
export function verificaValutaEFuso(account) {
  const problemi = [];
  if (account.currency !== 'EUR') problemi.push(`valuta ${account.currency || 'sconosciuta'} invece di EUR`);
  if (account.timezone_name !== 'Europe/Rome') problemi.push(`fuso ${account.timezone_name || 'sconosciuto'} invece di Europe/Rome`);
